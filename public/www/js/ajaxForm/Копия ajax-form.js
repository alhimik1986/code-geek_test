...
(function($){
	...
	ajaxForm = function(options) {
		...
		var _notValid = function(data, setting) {
			...
			setting.notValid(data, settings);
			settings.showLoading(false);
		};
		...
		var _send = function(setting) {
			...
			if ( ! settings.isset(ajaxOptions['ajax'])) setting.data = {};
		...
		var fnRowCallback = function( nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			var $nRow = $(nRow); var oldClass1 = '', oldClass2 = '';
		 ...
		 * @param object oData Данные таблицы в том же формате, что и для DataTables, НО аттрибуты строки таблицы содержатся в ключе "attributes".
		 * @param jQuery $tr jQuery-объект строки таблицы.
		 * @param object settings Настройки текущего объекта ajax-формы.
		 */
		this.defaults.dataTable.updateRow = function(oData, $tr, settings) {
			oData = oData[0];
			//var attributes = oData['attributes'];
			
			settings.dataTable.dom.fnUpdate(oData, $tr[0]);
