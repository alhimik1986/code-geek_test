...
/*	protected function rewriteLimitOffsetSql($sql, $limit, $offset)
	...
	// http://yiiframework.ru/forum/viewtopic.php?t=9633
	protected function rewriteLimitOffsetSql($sql, $limit, $offset)
	{
        //start page
        $startNum = $offset;
        ...
        $endNum = $offset + $limit;
        //print_r($sql);die;
        preg_match('@(?P<order>(ORDER BY)[\s"\[](.*)(ASC|DESC)?(?:[\s"\[]|$|COMPUTE|FOR))@i', $sql, $orders);
        $order = isset($orders['order']) ? $orders['order'] : 'ORDER BY id';
		
        $sql = preg_replace('@(?P<order>(ORDER BY)[\s"\[](.*)(ASC|DESC)?(?:[\s"\[]|$|COMPUTE|FOR))@i', '', $sql);
        
        $sql = preg_replace_callback('@(?<=SELECT)(?:\s+TOP\s+\w+)?(?P<select>[\s\S]+)(?=FROM)@i', function($matches) use ($order, $limit) {
			if (count($matches) && isset($matches['select']))
			{
				$select = $matches['select'];
				return " ROW_NUMBER() OVER({$order}) as NUM, {$select}";
			}
		}, $sql);
                
        $sql = "
            SET NOCOUNT ON;
            WITH PageNumbers as ($sql)
            SELECT TOP {$limit} * FROM PageNumbers WHERE NUM>{$startNum} AND NUM <={$endNum}";

        return $sql;
	}
	// https://github.com/Diego-Rocha/yii/commit/45db26530aab5d728c4688b9e2ed26fbf050ef89
/*	protected function rewriteLimitOffsetSql($sql, $limit, $offset)
	{
		$end = $limit + $offset;
		$start = $end - $limit + 1;
